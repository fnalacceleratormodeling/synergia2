...
Core_diagnostics::calculate_abs_mean(Bunch const& bunch)
{
  ...
  MPI_Allreduce(
    MPI_IN_PLACE, abs_mean.data(), 6, MPI_DOUBLE, MPI_SUM, bunch.get_comm());

  for (int i = 0; i < 6; ++i) abs_mean(i) /= bunch.get_total_num();
...
Core_diagnostics::calculate_sum2(Bunch const& bunch, karray1d const& mean)
{
  ...
  for (int i = 0; i < 5; ++i)
    for (int j = i + 1; j < 6; ++j) sum2(i, j) = sum2(j, i);
...
Core_diagnostics::calculate_mom2(Bunch const& bunch, karray1d const& mean)
{
  ...
  for (int i = 0; i < 6; ++i)
    for (int j = 0; j < 6; ++j) mom2(i, j) = sum2(i, j) / bunch.get_total_num();
