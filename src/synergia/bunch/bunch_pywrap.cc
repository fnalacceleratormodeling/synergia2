...
PYBIND11_MODULE(bunch, m)
{
  ...
  py::class_<Diagnostics, PyDiagnostics, std::shared_ptr<Diagnostics>>(
    ...
    .def(py::pickle(
      [](py::object self) { return py::make_tuple(self.attr("__dict__")); },
      [](py::tuple const& t) {
  ...
  py::class_<Diagnostics_bulk_track,
             Diagnostics,
             std::shared_ptr<Diagnostics_bulk_track>>(m,
  ...
  py::class_<Diagnostics_particles,
             Diagnostics,
             std::shared_ptr<Diagnostics_particles>>(m, "Diagnostics_particles")
  ...
  m.def(
    ...
    [](Distribution& dist,
      ...
      if (pc_info.ndim != 2 || pc_info.shape[0] != 6 || pc_info.shape[1] != 6) {
        throw std::runtime_error("populate_6d_truncated: "
                                 "covariances must be a 2d array of 6x6");
      }
  ...
  m.def("get_correlation_matrix",
        [](py::array_t<double> map,
          ...
          if (map.ndim() != 2 || map.shape(0) != 6 || map.shape(1) != 6)
            throw std::runtime_error("get_correlation_matrix(): "
          ...
          auto matrix =
            get_correlation_matrix(ka_map, arms, brms, crms, beta, rms_index);

          auto arr = py::array_t<double>({6, 6});

          for (int i = 0; i < 6; ++i)
            for (int j = 0; j < 6; ++j) arr.mutable_at(i, j) = matrix(i, j);
