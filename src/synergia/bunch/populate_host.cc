...
{  
    ...
    { 
        ...
        {
            VectorXcd sum = evect_matrix.col(first) 
                + evect_matrix.col(remaining[item]);    
        ...
        if (conj==-1) 
            throw std::runtime_error(
                    "failed to find a conjugate pair in "
                    "_get_correlation_matrix");       
        
        remaining.erase( 
                std::remove(remaining.begin(), remaining.end(), conj), 
                remaining.end() );

        MatrixXd tmp = 
            ( evect_matrix.col(first)*evect_matrix.col(first).conjugate().transpose() 
