...
Bunch_with_diagnostics::Bunch_with_diagnostics(Bunch_sptr bunch_sptr, Standard_diagnostics_actions_sptr diagnostics_actions_sptr):
...
Bunch_with_diagnostics::check_bunch_pointer_in_diagnostics() const
{
    ...
    const  std::list<Diagnostics_sptr > 
            steps_list=get_diagnostics_actions_sptr()->get_per_steps_diagnostics_list();
    if (!steps_list.empty()){
        for( std::list<Diagnostics_sptr >::const_iterator it=steps_list.begin(); it != steps_list.end();++it){
            if ((*it)->get_bunch_sptr() != bunch_sptr)  throw std::runtime_error(
     ...
     const  std::list<Diagnostics_sptr > 
            turns_list=get_diagnostics_actions_sptr()->get_per_turns_diagnostics_list();
    if (!turns_list.empty()){
        for( std::list<Diagnostics_sptr >::const_iterator it=turns_list.begin(); it != turns_list.end();++it){
            if ((*it)->get_bunch_sptr() != bunch_sptr)  throw std::runtime_error(
...
void
Bunch_with_diagnostics::add_per_step_diagnostics(Diagnostics_sptr diagnostics_sptr)
{
...
void
Bunch_with_diagnostics::add_per_turn_diagnostics(Diagnostics_sptr diagnostics_sptr)
{
