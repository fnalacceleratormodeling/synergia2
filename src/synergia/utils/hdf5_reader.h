...
public:
            ...
            int root_rank)
  {
    ...
    if (data_rank) {
      ...
      auto res = H5Sselect_hyperslab(
        fspace, H5S_SELECT_SET, offset.data(), NULL, di.dims.data(), NULL);
    ...
    if (collective) {
      ...
      if (mpi_rank == root_rank) {
        ...
        for (int r = 0; r < mpi_size; ++r) {
          ...
          if (r == mpi_rank) {
            auto res = H5Dread(
              dset, di.atomic_type, mspace, fspace, H5P_DEFAULT, (void*)di.ptr);
            if (res < 0) throw Hdf5_exception("error read");
          } else {
...
#else
    auto res = H5Ovisit(
      file, H5_INDEX_NAME, H5_ITER_NATIVE, cb, (void*)&names, H5O_INFO_BASIC);
#endif
