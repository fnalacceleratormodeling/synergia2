                        ...
                        int root_rank)
{
  ...
  if (data_rank) {
    ...
    herr_t res = H5Sselect_hyperslab(
      filespace2, H5S_SELECT_SET, offset.data(), NULL, dimsm.data(), NULL);
  ...
  if (mpi_rank == root_rank) {
    ...
    for (int r = 0; r < mpi_size; ++r) {
      ...
      if (data_rank) {
        ...
        herr_t res = H5Sselect_hyperslab(
          fspace2, H5S_SELECT_SET, offset.data(), NULL, dimsm.data(), NULL);
      ...
      if (r == mpi_rank) {
        // self
        herr_t res = H5Dwrite(
          dset, di.atomic_type, mspace, fspace2, H5P_DEFAULT, (void*)di.ptr);
        if (res < 0) throw Hdf5_exception();
      } else {
        ...
        if (sz) {
          ...
          MPI_Status status;
          MPI_Recv(
            (void*)buf.data(), buf.size(), MPI_BYTE, r, 0, comm, &status);
