...
BOOST_AUTO_TEST_CASE(sbend_propagation_libff)
{
    const double pc = 2.784435311;
    ...
    my_sbend.set_string_attribute("extractor_type", "libff");
    //my_sbend.set_string_attribute("extractor_type", "chef_propagate");
    ...
    // at offset x
    local_particles[3][Bunch::x] = x_offset;
    local_particles[3][Bunch::xp] = 0.0;
    local_particles[3][Bunch::y] = 0.0;
    local_particles[3][Bunch::yp] = 0.0;
    local_particles[3][Bunch::cdt] = 0.0;
    ...
    double m = pconstants::proton_mass;
    double p1 = p0 * (1.0 + dpp_offset);
    double E1 = sqrt(p1*p1 + m*m);
    double igamma1 = m / E1;
    double beta1  = sqrt(1.0 - igamma1 * igamma1); 
    ...
    BOOST_CHECK(floating_point_equal(d_offset, local_particles[2][Bunch::x], sbend_tolerance));

    // check px/pref
    double px_over_pref = dpp_offset * std::sin(sbend_angle);
    BOOST_CHECK(floating_point_equal(px_over_pref, local_particles[2][Bunch::xp], sbend_tolerance));
    ...
    BOOST_CHECK(floating_point_equal(delta_cdt, local_particles[2][Bunch::cdt], sbend_tolerance));
...
#endif
    BOOST_CHECK(floating_point_equal(local_particles[3][Bunch::x], d_offset, sbend_tolerance));
...
#endif
    BOOST_CHECK(floating_point_equal(local_particles[3][Bunch::cdt], delta_cdt, sbend_tolerance));
...
#endif
    BOOST_CHECK(floating_point_equal(local_particles[3][Bunch::xp], exit_px, sbend_tolerance));
