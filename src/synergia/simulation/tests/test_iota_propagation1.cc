...
beam, particle=proton, energy = 0.00250 + pmass;
    ...
    Lattice lattice(reader.get_lattice("iota"));
    for (auto& elem: lattice.get_elements())
      {
	elem.set_string_attribute("aperture_type", "circular");
	elem.set_double_attribute("circular_aperture_radius", 0.025);
      }
    return lattice;
}

Propagator
create_propagator(Lattice lattice, int nsteps=1)
{
...
TEST_CASE("propagate_particles_1")
{
  do_prop_test(1);
}

TEST_CASE("propagate_particles_2")
{
  do_prop_test(2);
}

TEST_CASE("propagate_particles_3")
{
  do_prop_test(3);
}

TEST_CASE("propagate_particles_4")
{
  do_prop_test(4);
}

TEST_CASE("propagate_particles_5")
{
  do_prop_test(5);
}
