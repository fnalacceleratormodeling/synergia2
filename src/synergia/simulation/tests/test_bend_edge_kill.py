...
def prop_fixture_nokick():
    channel_madx = """
beam, particle=proton,energy=0.8+pmass;
b: sbend, angle=pi/30.0, l=3.0, kill_entry_kick=1.0, kill_exit_kick=1.0;
channel: sequence, l=3.0, refer=entry;
b, at=0.0;
endsequence;
"""
...
def test_edge1(prop_fixture_edgekick, prop_fixture_nokick):
    # print('lattice: ')
    # print(prop_fixture.get_lattice())

    sim1 = create_simulator(prop_fixture_edgekick.get_lattice().get_reference_particle())
    sim2 = create_simulator(prop_fixture_nokick.get_lattice().get_reference_particle())
    bunch1 = sim1.get_bunch()
    bunch2 = sim2.get_bunch()
    ...
    lp2 = bunch2.get_particles_numpy()
    ...
    prop_fixture_nokick.propagate(sim2, simlog, nturns)
    ...
    for ix in range(5):
        for iy in range(5):
            ...
            if iy == 2:
                assert lp1[5*ix+2, 3] == pytest.approx(0.0, abs=1.0e-12)
            else:
                pass
