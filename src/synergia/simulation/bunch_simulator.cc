...
Bunch_simulator::prop_action_turn_end(Lattice& lattice, int turn)
{
  ...
  for (auto const& action : prop_actions_turn_end) action(*this, lattice, turn);
}
...
namespace {
  ...
  get_bunch_ptrs(std::array<Bunch_train, 2> const& trains)
  {
    ...
    for (auto const& t : trains)
      for (auto const& b : t.get_bunches()) bunches.push_back(&b);
  ...
  get_bunch_ptrs(std::array<Bunch_train, 2>& trains)
  {
    ...
    for (auto& t : trains)
      for (auto& b : t.get_bunches()) bunches.push_back(&b);
