...
namespace Lattice_simulator {
  ...
  tune_rfcavities(Lattice& lattice)
  {
    ...
    // go back and set the frequency of the cavities based on the accumulated
    // cdt
    double f = pconstants::c / accum_cdt;

    for (auto& ele : lattice.get_elements()) {
      if (ele.get_type() == element_type::rfcavity) {
        // set the frequency of the cavity if it doesn't already have one set
        // and there is a reasonable harmonic number
...
#include <gsl/gsl_vector.h>
  ...
  namespace {
                     ...
                     gsl_vector* co_results)
    {
      ...
      auto comm = Commxx();
      Bunch bunch(
        copp->lattice.get_reference_particle(), comm.size(), 1.0e10, comm);
...
#include "synergia/foundation/trigon.h"
  ...
  namespace {
    ...
    struct Adjust_tunes_params {
      ...
      Adjust_tunes_params(Lattice& lattice,
    ...
    adjust_tunes_function(const gsl_vector* x, void* params, gsl_vector* f)
    {
      ...
      auto nus =
        Lattice_simulator::calculate_tune_and_cdt(atparams->lattice, 0.0);
               ...
               double tolerance)
  {
    ...
    Adjust_tunes_params atparams(
      lattice, horizontal_tune, vertical_tune, h_correctors, v_correctors);
