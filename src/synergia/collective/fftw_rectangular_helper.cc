...
Fftw_rectangular_helper::Fftw_rectangular_helper(std::vector<int > const & grid_shape, Commxx const &comm_f):
    comm_f(comm_f), grid_shape( grid_shape)
{
    ...
    fftw_local_size = fftw_mpi_local_size_many(2, ndim, grid_shape[2],FFTW_MPI_DEFAULT_BLOCK, comm_f.get(), &local_nx, &local_x_start);
    if (local_nx == 0) {
    ...
    plan_r2r_direct=fftw_mpi_plan_many_r2r(2, ndim, grid_shape[2], FFTW_MPI_DEFAULT_BLOCK, FFTW_MPI_DEFAULT_BLOCK,
                                       data, data, comm_f.get(), kind_direct, FFTW_EXHAUSTIVE);  
   ...
   plan_r2r_inv=fftw_mpi_plan_many_r2r(2, ndim, grid_shape[2], FFTW_MPI_DEFAULT_BLOCK, FFTW_MPI_DEFAULT_BLOCK,
                                       data, data, comm_f.get(), kind_inv, FFTW_EXHAUSTIVE);                                    
 ...
 void
Fftw_rectangular_helper::reset_comm_f(Commxx const & comm)
{
    ...
    ptrdiff_t ndim[]={grid_shape[0],grid_shape[1]};
    fftw_local_size = fftw_mpi_local_size_many(2, ndim, grid_shape[2],FFTW_MPI_DEFAULT_BLOCK, comm.get(), &local_nx, &local_x_start);
    if (local_nx == 0) {
        have_local_data = false;
    } else {
        have_local_data = true;
    ...
    fftw_r2r_kind kind_direct[]={FFTW_RODFT10, FFTW_RODFT10};
    plan_r2r_direct=fftw_mpi_plan_many_r2r(2, ndim, grid_shape[2], FFTW_MPI_DEFAULT_BLOCK, FFTW_MPI_DEFAULT_BLOCK,
                                       data, data, comm.get(), kind_direct, FFTW_EXHAUSTIVE);  
                                       
                                       
    fftw_r2r_kind kind_inv[]={FFTW_RODFT01, FFTW_RODFT01};              
    plan_r2r_inv=fftw_mpi_plan_many_r2r(2, ndim, grid_shape[2], FFTW_MPI_DEFAULT_BLOCK, FFTW_MPI_DEFAULT_BLOCK,
                                       data, data, comm.get(), kind_inv, FFTW_EXHAUSTIVE); 
}
