...
BOOST_AUTO_TEST_CASE(interpolate_rectangular_xyz_basic)
{
    ...
    grid.get_domain_sptr()->get_cell_coordinates(0, 0, 0, x_left, y_left,
            z_left);
     double x_right, y_right, z_right;
     grid.get_domain_sptr()->get_cell_coordinates(1, 1, 1, x_right, y_right,
             z_right);
     double left_val = 2.0;
     ...
     // outside grid
    BOOST_CHECK(interpolate_rectangular_xyz(x_left - cell_size[0],
                     y_left, x_left, *grid.get_domain_sptr(),
                     grid.get_grid_points()) == 0.0);
    BOOST_CHECK(interpolate_rectangular_xyz(x_left,
                    y_left - cell_size[1], z_left, *grid.get_domain_sptr(),
                    grid.get_grid_points()) == 0.0);
    BOOST_CHECK(interpolate_rectangular_xyz(x_left,
                    y_left, z_left - cell_size[2], *grid.get_domain_sptr(),
                    grid.get_grid_points()) == 0.0);
                    
     BOOST_CHECK(interpolate_rectangular_xyz(x_left - 0.01*cell_size[0],
                     y_left, x_left, *grid.get_domain_sptr(),
                     grid.get_grid_points()) == 0.0);
    BOOST_CHECK(interpolate_rectangular_xyz(x_left,
                    y_left - 0.01*cell_size[1], z_left, *grid.get_domain_sptr(),
                    grid.get_grid_points()) == 0.0);
    BOOST_CHECK(interpolate_rectangular_xyz(x_left,
                    y_left, z_left - 0.01*cell_size[2], *grid.get_domain_sptr(),
                    grid.get_grid_points()) == 0.0);               
                    
                    

    BOOST_CHECK(interpolate_rectangular_xyz(
                    x_left + physical_size[0] + cell_size[0],
                    y_left, z_left, *grid.get_domain_sptr(),
                    grid.get_grid_points()) == 0.0);
    BOOST_CHECK(interpolate_rectangular_xyz(x_left,
                    y_left + physical_size[1] + cell_size[1],
                    z_left, *grid.get_domain_sptr(),
                    grid.get_grid_points()) == 0.0);
    BOOST_CHECK(interpolate_rectangular_xyz(x_left,
                    y_left,
                    z_left + physical_size[2] + cell_size[2],
                    *grid.get_domain_sptr(), grid.get_grid_points()) == 0.0);
                    
                    
     BOOST_CHECK(interpolate_rectangular_xyz(
                    x_left + physical_size[0] + 0.001*cell_size[0],
                    y_left, z_left, *grid.get_domain_sptr(),
                    grid.get_grid_points()) == 0.0);
    BOOST_CHECK(interpolate_rectangular_xyz(x_left,
                    y_left + physical_size[1] + 0.001*cell_size[1],
                    z_left, *grid.get_domain_sptr(),
                    grid.get_grid_points()) == 0.0);
    BOOST_CHECK(interpolate_rectangular_xyz(x_left,
                    y_left,
                    z_left + physical_size[2] + 0.001*cell_size[2],
                    *grid.get_domain_sptr(), grid.get_grid_points()) == 0.0);            
    ...
    for (double ds = 0.0; ds <= cell_size[2]; ds += cell_size[2] / num_steps) {
        ...
        double val = interpolate_rectangular_xyz(x_left, y_left, z_left+ ds,
                *grid.get_domain_sptr(), grid.get_grid_points());
        BOOST_CHECK_CLOSE(val, expected_val, strict_tolerance);
        // value along center of cube
        val = interpolate_rectangular_xyz((x_left + x_right) / 2.0,
                (y_left + y_right) / 2.0,  z_left + ds,
                *grid.get_domain_sptr(), grid.get_grid_points());
        BOOST_CHECK_CLOSE(val, expected_val, strict_tolerance);
    }
    ...
    for (double ds = 0.0; ds <= cell_size[1]; ds += cell_size[1] / num_steps) {
        double expected_val = left_val + dval_ds * ds;
        // value along edge of cube
        double val = interpolate_rectangular_xyz(x_left, y_left + ds, z_left,
                *grid.get_domain_sptr(), grid.get_grid_points());
        BOOST_CHECK_CLOSE(val, expected_val, strict_tolerance);
        // value along center of cube
        val = interpolate_rectangular_xyz((x_left + x_right) / 2.0,
    ...
    for (double ds = 0.0; ds <= cell_size[0]; ds += cell_size[0] / num_steps) {
        double expected_val = left_val + dval_ds * ds;
        // value along edge of cube
        double val = interpolate_rectangular_xyz(x_left+ ds, y_left, z_left,
                *grid.get_domain_sptr(), grid.get_grid_points());
        BOOST_CHECK_CLOSE(val, expected_val, strict_tolerance);
        // value along center of cube
        val = interpolate_rectangular_xyz(x_left + ds,  (y_left
                + y_right) / 2.0, (z_left + z_right) / 2.0, *grid.get_domain_sptr(),
                grid.get_grid_points());
        BOOST_CHECK_CLOSE(val, expected_val, strict_tolerance);
    }
...
BOOST_AUTO_TEST_CASE(interpolate_rectangular_xyz_basic_periodic)
{
    std::vector<double > physical_size(3);
    physical_size[0] = 2.0;
    physical_size[1] = 2.0;
    ...
    Rectangular_grid grid(physical_size, physical_offset, grid_shape, true);
    std::vector<double > cell_size(grid.get_domain_sptr()->get_cell_size());
    ...
    BOOST_CHECK(interpolate_rectangular_xyz(x_left,
     ...
     // on the grid, check z periodicity               
     BOOST_CHECK(interpolate_rectangular_xyz(
                    x_left + physical_size[0] + 0.001*cell_size[0],
                    y_left, z_left, *grid.get_domain_sptr(),
                    grid.get_grid_points()) == 0.0);
    BOOST_CHECK(interpolate_rectangular_xyz(x_left,
     ...
     BOOST_CHECK(interpolate_rectangular_xyz(x_left,
                     y_left, z_left -5* cell_size[2], *grid.get_domain_sptr(),
                     grid.get_grid_points()) == 1.);
                     
     BOOST_CHECK(interpolate_rectangular_xyz(x_right,
                     y_right, z_right +15* cell_size[2], *grid.get_domain_sptr(),
                     grid.get_grid_points()) == 1.);               
    ...
    for (double ds = 0.0; ds <= cell_size[2]; ds += cell_size[2] / num_steps) {
        double expected_val = left_val + dval_ds * ds;
        // value along edge of cube
        double val = interpolate_rectangular_xyz(x_left, y_left, z_left+ ds+6* physical_size[2],
                *grid.get_domain_sptr(), grid.get_grid_points());
        BOOST_CHECK_CLOSE(val, expected_val, strict_tolerance);
        // value along center of cube
        val = interpolate_rectangular_xyz((x_left + x_right) / 2.0,
                (y_left + y_right) / 2.0,  z_left + ds-12* physical_size[2],
                *grid.get_domain_sptr(), grid.get_grid_points());
        BOOST_CHECK_CLOSE(val, expected_val, strict_tolerance);
    }
    ...
    for (double ds = 0.0; ds <= cell_size[1]; ds += cell_size[1] / num_steps) {
        double expected_val = left_val + dval_ds * ds;
        // value along edge of cube
        double val = interpolate_rectangular_xyz(x_left, y_left + ds, z_left-3* physical_size[2],
                *grid.get_domain_sptr(), grid.get_grid_points());
        BOOST_CHECK_CLOSE(val, expected_val, strict_tolerance);
        // value along center of cube
        val = interpolate_rectangular_xyz((x_left + x_right) / 2.0,
                y_left + ds, (z_left + z_right) / 2.0-4* physical_size[2], *grid.get_domain_sptr(),
                grid.get_grid_points());
        BOOST_CHECK_CLOSE(val, expected_val, strict_tolerance);
    }
    ...
    for (double ds = 0.0; ds <= cell_size[0]; ds += cell_size[0] / num_steps) {
        double expected_val = left_val + dval_ds * ds;
        // value along edge of cube
        double val = interpolate_rectangular_xyz(x_left+ ds, y_left, z_left+2* physical_size[2],
                *grid.get_domain_sptr(), grid.get_grid_points());
        BOOST_CHECK_CLOSE(val, expected_val, strict_tolerance);
        // value along center of cube
        val = interpolate_rectangular_xyz(x_left + ds,  (y_left
                + y_right) / 2.0, (z_left + z_right) / 2.0+12* physical_size[2], *grid.get_domain_sptr(),
                grid.get_grid_points());
        BOOST_CHECK_CLOSE(val, expected_val, strict_tolerance);
    }
