...
BOOST_AUTO_TEST_CASE(get_global_charge_density2_reduce_scatter)
{
    ...
    Rectangular_grid_domain_sptr domain_sptr(new Rectangular_grid_domain(
            physical_size, physical_offset, grid_shape, false));
    space_charge.set_fixed_domain(domain_sptr);
    Rectangular_grid_domain_sptr doubled_domain_sptr(
            space_charge.get_doubled_domain_sptr());
    Rectangular_grid_sptr local_rho(new Rectangular_grid(doubled_domain_sptr));
    std::vector<int > doubled_grid_shape(
            local_rho->get_domain_sptr()->get_grid_shape());
    // local_rho will be overwritten by the in-place allreduce, so we
    // keep a copy for comparison.
    Rectangular_grid_sptr local_rho_orig(new Rectangular_grid(
            doubled_domain_sptr));
    for (int i = 0; i < doubled_grid_shape[0]; ++i) {
        for (int j = 0; j < doubled_grid_shape[1]; ++j) {
            local_rho->get_grid_points_2dc()[i][j] = i + 10 * j;
        }
    }
    for (int k = 0; k < doubled_grid_shape[2]; ++k) {
        local_rho->get_grid_points_1d()[k] = 11 * k;
    ...
    BOOST_CHECK_EQUAL(doubled_grid_shape[2], doubled_shape[2]);
...
BOOST_AUTO_TEST_CASE(get_global_charge_density2_allreduce)
{
    ...
    for (int i = 0; i < doubled_grid_shape[0]; ++i) {
        for (int j = 0; j < doubled_grid_shape[1]; ++j) {
            local_rho->get_grid_points_2dc()[i][j] = i + 100 * j;
            local_rho_orig->get_grid_points_2dc()[i][j] = i + 100 * j;
        }
    }
    for (int k = 0; k < doubled_grid_shape[2]; ++k) {
        local_rho->get_grid_points_1d()[k] = 11 * k;
