...
BOOST_AUTO_TEST_CASE(construct)
{
    ...
    BOOST_CHECK_EQUAL(space_charge.get_have_fftw_helper(), true);
    BOOST_CHECK_EQUAL(space_charge.get_have_domain(), false);
...
BOOST_AUTO_TEST_CASE(construct1)
{
    ...
    BOOST_CHECK_EQUAL(space_charge.get_have_fftw_helper(), false);
    BOOST_CHECK_EQUAL(space_charge.get_have_domain(), false);

}

BOOST_FIXTURE_TEST_CASE(set_domain, Ellipsoidal_bunch_fixture)
{
    ...
    BOOST_CHECK_EQUAL(space_charge.get_have_domain(), false);
    space_charge.set_domain(bunch);
    BOOST_CHECK_EQUAL(space_charge.get_have_domain(), true);
    BOOST_CHECK_CLOSE(size[0], space_charge.get_domain().get_physical_size()[0],tolerance );
    BOOST_CHECK_CLOSE(size[1], space_charge.get_domain().get_physical_size()[1],tolerance );
    BOOST_CHECK_CLOSE(size[2]/beta, space_charge.get_domain().get_physical_size()[2],tolerance );
...
BOOST_FIXTURE_TEST_CASE(get_local_charge_density, Ellipsoidal_bunch_fixture)
{
  ...
  //  BOOST_CHECK_CLOSE(rho->get_normalization(), local_charge, 100*tolerance);
    ...
    for (int i = center[0] - 1; i < center[0] + 1; ++i) {
        for (int j = center[1] - 1; j < center[1] + 1; ++j) {
            for (int k = center[2] - 1; k < center[2] + 1; ++k) {
                expected[i][j][k] = 0.125 * density_norm*size_com*beta;
            }
        }
    }
...
BOOST_FIXTURE_TEST_CASE(get_phi_local, Ellipsoidal_bunch_fixture)
{
    ...
    space_charge.set_fftw_helper(bunch.get_comm_sptr(), false);
    Distributed_rectangular_grid_sptr phi_local(space_charge.get_phi_local(*rho,bunch.get_reference_particle().get_gamma()));
...
//     space_charge.set_fftw_helper(bunch.get_comm_sptr());
	...
	Distributed_rectangular_grid_sptr phi_local(space_charge.get_phi_local(*rho_grid,gamma));
	...
	double zpt=2.*pi*pt/(size[2]*gamma);
 ...
 BOOST_FIXTURE_TEST_CASE(get_En, Ellipsoidal_bunch_fixture)
{
   ...
   std::vector<Rectangular_grid_sptr>  Efield(space_charge.get_Efield(*rho_grid, bunch,3,bunch.get_reference_particle().get_gamma()));
...
BOOST_FIXTURE_TEST_CASE(serialize_comm_divided, Ellipsoidal_bunch_fixture)
{
   ...
   space_charge0.set_domain(bunch);
   int optimal_number=2;
   if (bunch.get_comm_sptr()->get_size() %  optimal_number==0) {
	Commxx_sptr comm_spc=make_optimal_spc_comm(bunch.get_comm_sptr(), optimal_number,true);	
