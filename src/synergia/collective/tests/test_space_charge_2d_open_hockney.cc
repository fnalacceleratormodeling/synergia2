...
// same as real_apply_transverse_lowgamma except the bunch is half length for double
...
BOOST_FIXTURE_TEST_CASE(real_apply_transverse_compressed_lowgamma, Rod_bunch_fixture_compressed_lowgamma)
{
    ...
    logger << "after sc::apply : bunch.get_local_particles()[0][0]: "
           << bunch.get_local_particles()[0][0] << std::endl;
    // Rod of length L, charge Q charge is confined to the middle half
    // E field at radius r $$ E = \frac{1}{2 \pi \epsilon_0} \frac{Q}{(L/2)}
    // \frac{1}{r} $$
    // B field at radius r $$ E = \frac{\mu_0}{2 \pi } \frac{Q v}{(L/2)} \frac{1}{r}
    // $$
    // Net EM force on electric+magnetic on probe of charge q from E-B
    // cancellation
    // $$ F = \frac{1}{2 \pi \epsilon_0 \gamma^2} \frac{qQ}{(L/2)} \frac{1}{r}
    // travel over distance D at velocity v
    // \frac{\Delta p}{p} = \frac{1}{2 \pi \epsilon_0 \gamma^2} \frac{qQ}{(L/2)}
    // \frac{D}{m v^2} \frac{1}{r}
    // convert to usual units
    // \frac{\Delta p}{p} = \frac{2 N r_p}{(L/2) \beta^2 \gamma^3} \frac{D}{r}

    // the charge takes up only half the actual bucket length
    double L = bunch.get_z_period_length()/2.0;
    double N = bunch.get_real_num();
    logger << "L: " << L << std::endl;
    logger << "N: " << N << std::endl;
    logger << "step_length: " << step_length << std::endl;
...
// same as real_apply_transverse_highgamma except the charge is compressed to
//  half the length so double the line density
