...
namespace
{
    ...
    unit_efield(double x, double y, 
            bool linear, double sigma_x, double sigma_y)
    {
...
Space_charge_2d_kv::Space_charge_2d_kv(
        Space_charge_2d_kv_options const& opts) 
    : Collective_operator("sc_2d_kv", 1.0)
    , opts(opts)
{
}

void 
Space_charge_2d_kv::apply_impl(
            Bunch_simulator& sim, 
            double time_step, 
            Logger& logger)
{
...
void
Space_charge_2d_kv::apply_bunch(
            Bunch& bunch, 
            double time_step, 
            Logger& logger)
{
    ...
    double factor = pconstants::rp * pconstants::c 
        * time_step / (gamma*gamma*gamma*beta);
            ...
            Space_charge_2d_kv_options::LD::uniform)
    {
        ...
        sc_kv_uniform alg{ parts, masks, 
            offx, offy, std(0), std(2),
            factor, line_charge_density, 
    ...
    {
        double coeff = total_q / 
            (sqrt(2.0 * mconstants::pi) * std(4) * beta);

        sc_kv_gaussian alg{ parts, masks, 
            offx, offy, offz, 
            std(0), std(2), std(4),
            factor, coeff, 
            opts.strictly_linear };
