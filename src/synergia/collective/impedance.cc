  ...
  struct alg_z_wake_reduce {
    ...
    operator()(const int j, value_type& sum) const
    {
      ...
      if (iz + 1 < size_wake) {
        ...
        double xwt =
          xw_trail[iz] + z1 * (xw_trail[iz + 1] - xw_trail[iz]) * recip_z2;
        ...
        double ywt =
          yw_trail[iz] + z1 * (yw_trail[iz + 1] - yw_trail[iz]) * recip_z2;
  ...
  struct alg_bunch_wake {
    ...
    operator()(const int i) const
    {
      ...
      for (int j = 0; j < num_bunches; ++j) {
        ...
        double zji = z_to_zmean + bunch_spacing * (bp.bucket - j_bucket) +
    ...
    operator()(const int i, value_type sum) const
    {
      ...
      for (int j = 0; j < num_bunches; ++j) {
        ...
        double zji = bunch_spacing * (bp.bucket - j_bucket) +
                     orbit_length * turn + (bps.zmean[j_idx] - bp.z_mean);
...
Impedance::calculate_kicks(Bunch const& bunch, Bunch_params const& bp)
{
  ...
  if (opts.full_machine) {
    ...
    /// exemple: full_machine, all bunches identical, no wave across the
    /// machine: num_trains=num_buckets, wn=[0,0,0], it's a one bunch simulation
    ...
    if (opts.wn[0] < 0 || opts.wn[0] >= num_trains || opts.wn[1] < 0 ||
        opts.wn[1] >= num_trains || opts.wn[2] < 0 || opts.wn[2] >= num_trains)
      throw std::runtime_error(
  ...
  if (bps.registered_turns > 1) {
    ...
    alg_turn_wake ft_turn_wake{bp,
                               ...
                               opts.orbit_length,
                               opts.full_machine};
    Kokkos::parallel_reduce(bps.registered_turns - 1, ft_turn_wake, turn_wakes);
