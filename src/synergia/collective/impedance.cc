        ...
        for (it=jt,  iturn=1; it !=stored_vbunches.end(); ++it, ++iturn){
            ...
            if (full_machine) {
                for (int itrain= 1; itrain<num_trains; ++itrain){
                    double wnx=cos(2.*Kokkos::numbers::pi_v<double>*wn[0]*itrain/double(num_trains));
                    double wny=cos(2.*Kokkos::numbers::pi_v<double>*wn[1]*itrain/double(num_trains));
                    double wnz=cos(2.*Kokkos::numbers::pi_v<double>*wn[2]*itrain/double(num_trains));
                    for (int ibunch= 0; ibunch<numbunches; ++ibunch){
