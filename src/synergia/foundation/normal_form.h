...
NormalForm<order>::NormalForm(mapping_t const& one_turn_map,
                              ...
                              double pc0,
                              double mass)
  : E_(Matrix6C::Zero()), invE_(Matrix6C::Zero()), f_(order - 1), g_(order - 1)
{
  ...
  KOKKOS_IF_ON_HOST((
    ...
    if (eigensolver.info() !=
        Eigen::Success) throw std::runtime_error("failed solving eigenvectors");
    ...
    for (int k = 0; k < order - 1; ++k) {
      ...
      for (int d = 0; d < dim; ++d) {
        ...
        doc[d].each_term([this, k, d, &lambda, &N, &T](
          ...
          for (int i = 0; i < power; ++i) factor *= complex_1 / lambda[ind[i]];
