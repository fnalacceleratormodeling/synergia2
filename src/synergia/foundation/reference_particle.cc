...
Reference_particle::Reference_particle() :
    charge(0), four_momentum(1.0, 1.0), state(boost::extents[6]),
            repetition(0), repetition_length(0), s_n(0)
{
...
Reference_particle::Reference_particle(int charge, double mass,
    ...
    charge(charge), four_momentum(mass, total_energy),
            state(boost::extents[6]), repetition(0), repetition_length(0), s_n(0)
{
    for (int i = 0; i < 6; ++i) {
        state[i] = 0;
    }
...
{
    return repetition * repetition_length + s_n;
}
