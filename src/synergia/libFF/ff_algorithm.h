...
#define MODEL 10
                  ...
                  int kicks)
  {
    ...
    if (kicks == 1) {
      ...
      for (int i = 0; i < u; ++i) {
        drift_unit(
          x, xp, y, yp, cdt, dpop, frontLength, pref, m, substep_ref_cdt);
        thin_quadrupole_unit(x, xp, y, yp, kl); // quaterStrength

        for (int i = 0; i < 3; ++i) {
          drift_unit(
            x, xp, y, yp, cdt, dpop, sepLength, pref, m, substep_ref_cdt);
          thin_quadrupole_unit(x, xp, y, yp, kl); // quaterStrength
        }
      ...
      drift_unit(
        x, xp, y, yp, cdt, dpop, len / (2.0 * kicks), pref, m, substep_ref_cdt);
      thin_quadrupole_unit(x, xp, y, yp, kl); // str*len/kicks

      for (int i = 0; i < kicks - 1; ++i) {
        drift_unit(
          x, xp, y, yp, cdt, dpop, len / kicks, pref, m, substep_ref_cdt);
        thin_quadrupole_unit(x, xp, y, yp, kl); // str*len/kicks
      }
...
template <typename T>
  ...
  struct yoshida_element<T, kf, 0, components> {
             ...
             double c)
    {
      ...
      for (int i = 0; i < components * 2; ++i) kl[i] = step_strength[i] * c;
  ...
  KOKKOS_INLINE_FUNCTION struct yoshida_element<T, kf, 0, components> {
             ...
             double c)
    {
      ...
      drift_unit(
        x, xp, y, yp, cdt, dpop, c * step_length, pref, m, substep_ref_cdt);
