...
double FF_vkicker::get_reference_cdt(double length, double k, Reference_particle &reference_particle, bool simple)
{
    ...
    else
    {
        if (simple)
        {
            ...
            FF_algorithm::thin_kicker_unit(yp, k * length);
...
void FF_vkicker::apply(Lattice_element_slice const& slice, Bunch& bunch)
{
    ...
    if ( close_to_zero(length) )
    {
        ...
        for (int part = 0; part < block_last; part += gsvsize)
        {
            ...
            FF_algorithm::thin_kicker_unit(yp, b_k);
            yp.store(&ypa[part]);
        }
    ...
    else
    {
        ...
        else
        {
            ...
            double step_length = length / steps;
            double step_strength[2] = { 0.0, b_k_pul * step_length};
