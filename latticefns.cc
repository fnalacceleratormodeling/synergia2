...
void insert_markers(BmlPtr bmline, int num_markers_per_element,
			     double momentum)
{
  //~ bmlnElmnt* element;
  //~ DeepBeamlineIterator deep_beamline_iterator(*bmline);
  marker accuracy_marker = marker("accuracy marker");
  double master_insertion_point = 0.0;
  double marker_interval;
  //~ InsertionList insertion_list(momentum);
   std::list<std::pair<ElmPtr,double> > insertion_list;
  //~ while((element = deep_beamline_iterator++)) {
  for(beamline::deep_iterator iter = bmline->deep_begin();
        iter != bmline->deep_end(); ++iter) {
    ElmPtr element = *iter;
    if (element->Length() > 0 ) {
	...
	//~ insertion_list.Append(new InsertionListElement(insertion_point,
						       //~ accuracy_marker));
	insertion_list.push_back(std::make_pair(static_cast<ElmPtr>(&accuracy_marker),
                insertion_point));
      }
  ...
  bmline->InsertElementsFromList(my_proton, s_0, insertion_list);
...
int main(int argc, char **argv)
{
  ...
  BmlPtr bmline_orig = bml_fact->create_beamline(line_name.c_str());
  bmline_orig->flatten();
  BmlPtr bmline(DriftsToSlots(*bmline_orig));

  insert_markers(bmline, 10, momentum);

  Proton jfcproton(energy);
  BeamlineContext bmln_context(jfcproton,bmline);
