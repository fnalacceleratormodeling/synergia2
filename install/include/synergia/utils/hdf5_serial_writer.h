...
private:
    ...
    template<typename T>
    std::vector<hsize_t> pre_setup(T const& data)
    {
        data_rank = T::Rank;
...
template<>
inline std::vector<hsize_t> Hdf5_serial_writer::pre_setup<int>(int const& data)
{
    data_rank = 0;
...
template<>
inline std::vector<hsize_t> Hdf5_serial_writer::pre_setup<double>(double const& data)
{
    data_rank = 0;
    atomic_type = hdf5_atomic_data_type<double>();
