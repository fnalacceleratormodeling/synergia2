...
verter" << endl;
  ...
  if( !(Sage::isRing( flattened )) ) {
    cout << "Whoops!  Not a RING!!!!" << endl;
    return -1;
  }
  ...
  // make sure we are on the closed orbit
  cout << "Reference momentum closed orbit check" << endl;
  ...
  // Set to off-momentum
  probe.SetReferenceMomentum( (1.0 + myOptions.dpp) * probe.ReferenceMomentum() );
  probe.setStateToZero();
  ...
  // michelotti: eta * dp/p = dt/t
  //                        = c dt/ct
  //                        = beta c dt / beta c t
  //                        = beta c dt / circumference

  double delta_beta_cdt = probe.get_cdt()*probe.Beta();
  cout << "delta beta cdt = " << delta_beta_cdt << endl;

  double slip = (delta_beta_cdt/circumference) / myOptions.dpp;

  cout << "Slip factor: " << slip << endl;
  ...
  double compactfact = slip + 1/(pgamma*pgamma);
  cout << "compaction factor (delta C/C)/(delta_p/p) = eta+1/gamma**2: " << compactfact << endl;
