...
elif (opts.latticefile=="booster_2012.lat")  or (opts.latticefile=="booster_zerochrom.lat") or (opts.latticefile=="booster_expchrom.lat"):
    lattice=synergia.lattice.Mad8_reader().get_lattice("machine", opts.latticefile)
    ...
    for elem in lattice.get_elements():
        if  (opts.latticefile=="booster_0.5.lat") or  (opts.latticefile=="booster_classic.lat")or \
            ...
            name1=elem.get_name()[0:3]
              ...
              name= elem.get_name()[1:5]
              name1=elem.get_name()[0:3]
        elif  (opts.latticefile=="booster_2012.lat") or (opts.latticefile=="booster_zerochrom.lat") or \
...
crms=opts.zrms  
#actions = stepper.get_lattice_simulator().get_stationary_actions(arms, brms, crms)
if MPI.COMM_WORLD.Get_rank() ==0:
...
bunch= synergia.optics.generate_matched_bunch(stepper.get_lattice_simulator(),
                                                ...
                                                seed=opts.seed,  periodic=opts.periodic)
...
if MPI.COMM_WORLD.Get_rank() ==0:
    print " bunch real num=", bunch.get_real_num()
    print " bunch macro num=", bunch.get_total_num()


bunch_simulator = synergia.simulation.Bunch_simulator(bunch)
bunch_simulator.add_per_step(synergia.bunch.Diagnostics_full2("test_step_full2.h5"))
bunch_simulator.add_per_turn(synergia.bunch.Diagnostics_particles("test_turn_particles.h5"))


if space_charge:
    
    #commx=bunch.get_comm()
    commx=Commxx()    
...
propagator.propagate(bunch_simulator, opts.num_turns, opts.maxturns, opts.verbose)                           
