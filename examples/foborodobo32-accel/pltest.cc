...
run()
{
   ...
   // stepper
    ...
    screen(LV::INFO) << "lattice.get_elements() first element orig a1 parameter: " << 
        lattice.get_elements().front().get_double_attribute("a1") << std::endl;
    lattice.get_elements().front().set_double_attribute("a1", 9999.0);
    screen(LV::INFO) << "lattice.get_elements() first element orig a1 parameter: " << 
        lattice.get_elements().front().get_double_attribute("a1") << std::endl;
    screen(LV::INFO) << "demonstrated that I can control lattice element attributes from lattice.get_elements()\n" << std::endl;
    ...
    "Initial a1 param propagator.get_lattice_elements().front(): " <<
        ...
        std::endl;
    screen(LV::INFO) << "propagator.get_lattice_elements() returns a const reference to lattice elements that are different than the original lattice used in propagator.  You can't modify any elements through this reference.\n" << std::endl;

    // Doesn't work because of const
    // propagator.get_lattice_elements().front().set_double_attribute("a1", -888.0);
    // screen(LV::INFO) << 
    // "After mod a1 param propagator.get_lattice_elements().front()" <<
    //     propagator.get_lattice_elements().front().get_double_attribute("a1") <<
    //     std::endl;

    Propagator::Lattice_element_slices les = propagator.get_lattice_element_slices();
    auto& first_slice = *les.begin();

    // By the way, Lattice_element_slice::get_lattice_element is a const& so can't
    // modify element?
    screen(LV::INFO) << "First slice:\n" << 
        first_slice.get_lattice_element().get_name() << ": a1: " <<
        first_slice.get_lattice_element().get_double_attribute("a1") <<
        std::endl;
    ...
    screen(LV::INFO) << "propagator.get_lattice().get_elements().front() initial a1 param l.get_elements().front(): " <<
        propagator.get_lattice().get_elements().front().get_double_attribute("a1") << std::endl;
    propagator.get_lattice().get_elements().front().set_double_attribute("a1", -555.0);
    screen(LV::INFO) << "propagator.get_lattice().get_elements().front() after mod a1 param l.get_elements().front(): " <<
        propagator.get_lattice().get_elements().front().get_double_attribute("a1") << std::endl;
