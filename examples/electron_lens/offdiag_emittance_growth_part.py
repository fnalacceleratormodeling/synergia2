...
def calculate_emittances(p):
    ...
    # these are the lattice functions at the boc element for
    # lattice tunes 3.72, 3.84
...
if __name__ == "__main__":
    ...
    nemit = len(xemit99)
    emittances = np.vstack( (
        np.array(turns),
        np.array(xemitrms), np.array(xemit95), np.array(xemit99), np.array(xemit999),
        np.array(yemitrms), np.array(yemit95), np.array(yemit99), np.array(yemit999) ) ).transpose()
    np.save("emittances", emittances)
