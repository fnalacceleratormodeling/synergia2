...
#include "cxx_idealcomp_options.h"

//-----------------------------------------------------------------------------------

void activate_elens(Cxx_idealcomp_options const& opts, Stepper_sptr stepper_sptr, int rank)
{
...
Lattice_sptr setup_lattice(Cxx_idealcomp_options const& opts, int rank)
{
   ...
   // determine type of propagation
    ...
    for (Lattice_elements::const_iterator it =
        ...
        // add forced diagnostics to measurement locations
        if (element_name == opts.measurepoint) {
...
void activate_error_element(Cxx_idealcomp_options const& opts, int rank, Stepper_sptr stepper_sptr, double nu_x, double nu_y)
{
    Lattice_sptr lattice_sptr(stepper_sptr->get_lattice_simulator().get_lattice_sptr());
...
Collective_operators get_solver(Cxx_idealcomp_options const& opts, int rank)
{
    ...
    if(opts.solver == "3doh") {
        ...
        if (opts.magiccomp > 0) {
            ...
            comp3dh_sptr->set_green_fn_type(Space_charge_3d_open_hockney::linear);
    ...
    for (Steps::iterator sit=stepper_sptr->get_steps().begin(); sit!=stepper_sptr->get_steps().end(); ++sit, ++stepcnt) {
        ...
        for (Operators::iterator oit=(*sit)->get_operators().begin(); oit!=(*sit)->get_operators().end(); ++oit) {
            ...
            if ((*oit)->get_name() == "space charge 3D open hockney") {
                ...
                std::cout << "Space charge operator scale: " << ks << std::endl;
...
// activate magiccomp
void
activate_magiccomp(Cxx_idealcomp_options const& opts, Stepper_sptr stepper_sptr, int rank)
{
    if (opts.magiccomp == 1) {
...
void
run(Cxx_idealcomp_options const& opts)
{
...
#if 0
    // use the choice stepper.  There will be one space charge kick for
    // each electron lens.  The "else" kicks will be dummy kicks.
    // this doesn't work
    if (opts.stepper == "choice") {
        // if ( (opts.space_charge_2dh) || (opts.space_charge_3dh) || (opts.space_charge_rec) || (opts.bpms) ) {
        List_choice_map list_choice_map;

        Collective_operators operators_elens;

        if (opts.space_charge_3dh) operators_elens.push_back(sc_solver_sptr);
        Kicks  kicks_elens(operators_elens,1); // one space charge kick per lens

        Collective_operators operators_else;
        operators_else.push_back(Dummy_collective_operator_sptr(
            new Dummy_collective_operator("dummy")));
        Kicks kicks_else(operators_else, 200);

        //list_choice_map["f"] = kicks_else;
        //list_choice_map["o"] = kicks_else;
        //list_choice_map["d"] = kicks_else;
        //list_choice_map["r"] = kicks_else;
        list_choice_map["oscelem"] = kicks_elens;
       ...
       stepper_sptr=Stepper_sptr(new Split_operator_stepper_choice(12, lattice_sptr, opts.map_order, list_choice_map, true));
       // stepper_sptr=Stepper_sptr(new Split_operator_stepper_choice(1, lattice_sptr, opts.map_order, list_choice_map, false));
        if (rank==0){
            std::cout<<std::endl;
    ...
    if (opts.stepper == "splitoperator") {
        ...
        if (rank == 0) {
            std::cout << std::endl;
            std::cout << "Split_operator stepper created, steps/line: " << opts.num_steps << std::endl;
            std::cout<<"___________________________________________________________"<<std::endl;
        }
    } else {
    ...
    if (rank == 0) {
        ...
        for (Lattice_elements::const_iterator it = lattice_sptr->get_elements().begin();
            it != lattice_sptr->get_elements().end(); ++it) {
            if ((*it)->get_name() == opts.measurepoint) {
                Lattice_functions lf(stepper_sptr->get_lattice_simulator().get_lattice_functions(**it));
                std::cout << "element " << (*it)->get_name() << "@: " << lf.arc_length << ", betax: " << std::setprecision(15) << lf.beta_x << ", alphax: " << lf.alpha_x << ", betay: " << lf.beta_y << ", alphay: " << lf.alpha_y << std::endl;
            }
        }
    }
    ...
    if (rank == 0) {
        std::cout << "\nLattice functions at beginning of ring" << std::endl;
        std::cout << "begin: " << lf.arc_length << ", betax: " << std::setprecision(15) << lf.beta_x << ", alphax: " << lf.alpha_x << ", betay: " << lf.beta_y << ", alphay: " << lf.alpha_y << std::endl;
    }
    ...
    // does the space charge operator actually have diagnostics somehow?
    for (Steps::const_iterator it = stepper_sptr->get_steps().begin(); it != stepper_sptr->get_steps().end(); ++it) {
        for (Operators::const_iterator o_it = (*it)->get_operators().begin(); o_it
                     != (*it)->get_operators().end(); ++o_it) {
            if ((*o_it)->get_name()=="space charge 3D open hockney") {
                (*o_it)->print();
                std::cout << "3d_open_hockney operator -> has_diagnostics(): " <<
                    boost::dynamic_pointer_cast<Space_charge_3d_open_hockney>(*o_it)->has_diagnostics() << std::endl;
            }
        }
    }
#endif
...
{
   MPI_Init(&argc, &argv);
   Cxx_idealcomp_options opts(argc, argv);
  run(opts);
   MPI_Finalize();
    return 0;
}
