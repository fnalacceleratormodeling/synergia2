...
run()
{
  ...
  for (int i = 0; i < macro_particles; ++i)
    for (int j = 0; j < 6; ++j) hparts(i, j) = 0.0;

  for (int i = 0; i < macro_particles / 2; ++i) hparts(i, 0) = 0.001 * i;

  for (int i = macro_particles / 2; i < macro_particles; ++i)
    hparts(i, 2) = 0.001 * (i - macro_particles / 2.0);
  ...
  // propagate actions
  sim.reg_prop_action_turn_end(
    [&screen, &nf, &of](auto& sim, auto& lattice, int turn) {
      // screen << "turn = " << turn << "\n";
      auto& bunch = sim.get_bunch();
      auto hparts = bunch.get_host_particles();
      ...
      for (int i = 0; i < nparts; ++i) {
        ...
        of << nfd[0].real() << " " << nfd[0].imag() << " " << nfd[1].real()
           << " " << nfd[1].imag() << " " << nfd[2].real() << " "
           << nfd[2].imag() << " ";
      }
