...
#    scaling_frequency = 10221.05558e6 # Angular frequency 
    ...
    tokenCase="V3e11ppbMediumPNegLW2" # MediumP means 2.0e-8 
...
#    griddimA = numpy.array([griddim[0], griddim[1], griddim[2]])
    ...
    plotPotential.plotPotentialX(phiAfterProp, totalQ, "SB1")
...
#
    bYDip=0.05 # well make this a bit more variable... 
    ...
    mEl.propagateWithBeam(False, phiAfterProp, totalQ, tokenTraj)
    print " Number of electron in Vacuum, after with Beam ", mEl.numInVaccum()
...
#    sys.exit(2)
    ...
    mEl.pyplotEk1(tokenNow, True, 50.)
    mEl.pyplotEk1(tokenNow, False, 5.0)
    print " Regeneration at the wall " 
    mEl.AddFromWall(2.0, 1) # 0 is for copper, 1 for Stainless Steel
    # Propagate again, in case we have quickly created additional electrons.
    print " Electron counter ", mEl.electronCounter()
    print " Number of electron in Vacuum ", mEl.numInVaccum()
    print " Propagating again within the bunch after regeneration on the wall "   
    tokenNow=tokenCase+"WallElectron"
    mEl.setNumberTrajectoryDump(10)
    mEl.propagateWithBeam(False, phiAfterProp, totalQ, tokenTraj)
    mEl.setNumberTrajectoryDump(2)
    print " Electron counter ", mEl.electronCounter()
    print " Number of electron in Vacuum ", mEl.numInVaccum()
#    print " Done for now.. " 
#    sys.exit(2)
    print " Propagating in between bunches..." 
    mEl.propagateNoBeam(tokenTraj) # Until the next bunch crossing, approximately 9 ns 
    print " Number of electron in Vacuum ", mEl.numInVaccum()
    print " Number of electron lost in propagation error  ", mEl.nBad
    print " Number of electron Reached Beam Pipe ", mEl.numReachedBeamPipe()
    print " Average Time of flight in vaccum ", mEl.averageTimeOfFlight()
    print " Average kinetic energy for my electron in vaccum ", mEl.averageKineticEnergy()
    print " Average kinetic energy hitting Beam Pipe ", mEl.averageKineticEnergyBP()
    print " Average Time of flight hitting Beam Pipe ", mEl.averageTimeOfFlightBP()
    print " Average radius... ", mEl.averageRadius()
    print " Average radial velocity ", mEl.averageRadialVelocity()
    mEl.writeToASCIIFile("electronFlockPartialCycleV1.txt")
#
# Collect again the electron on the wall, secondary emission and propagation again. 
    mEl.AddFromWall(2.0, 1) # 0 is for copper, 1 for Stainless Steel 
    mEl.propagateNoBeam(tokenTraj) # Until the next bunch crossing... 
    print " Number of electron in Vacuum ", mEl.numInVaccum()
    print " Number of electron lost in propagation error  ", mEl.nBad
    print " Number of electron Reached Beam Pipe ", mEl.numReachedBeamPipe()
    print " Average kinetic energy for my electron in vaccum ", mEl.averageKineticEnergy()
    ...
    print " Average radius... ", mEl.averageRadius()
    print " Average radial velocity ", mEl.averageRadialVelocity()
# Try one more thing: complete crossing through a bunch, in Steady State. 
    mEl.resetClockForNextBunch()
    mEl.propagateWithBeam(True, phiAfterProp, totalQ, tokenTraj)
    mEl.writeToASCIIFile("electronFlockWithBeamSteadyV2.txt")
    
    tokenNow=tokenCase+"Final"
    mEl.pyplotEk1(tokenNow, True, 50.)
