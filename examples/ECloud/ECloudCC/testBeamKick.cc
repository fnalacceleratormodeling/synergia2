...
int  main (void)
{
  // Must do this, since we assign random velocity to our particle..
  setGSLRandom();
  ...
  for (int i=0; i!=500; i++) {
    ...
    for (int k=0; k !=3; k++) fOutC1 << " " <<  ETmp1[k]; 
    fOutC1   << std::endl;			      
  }
  fOutC1.close();
...
//  exit(2); 
  ...
  double y[6] = { 0.0010, betax , 0.0010, betay , -0.25, betaz};
...
//  myRK.setPrecisionStep(1.0e-10);
  ...
  myRK.setTrajectoryFileName("TrajBeamKickBiGaussV3aj.txt");
  myRK.setMaximumXBeamPipe(5.5e-2);
  myRK.setMaximumYBeamPipe(2.5e-2);
  // Test at 5 tesla... 
  myRK.setFieldModel(RKIntegrator::UNIFORM, 5.0);
  myRK.setBFieldStaticCmp(0., 0);
  myRK.setBFieldStaticCmp(5., 1);
  myRK.setBFieldStaticCmp(0., 0);
  ...
  if (explicitInterface) { 
    ...
    if (bFieldOn) {
      ...
      myRK.setBFieldStaticCmp(5., 1); // Test at 5 tesla, see above... 
