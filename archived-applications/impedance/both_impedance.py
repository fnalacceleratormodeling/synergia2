...
def propagate():
    ...
    for i in range(num_bunches):
        ...
        lp[1:50001, 0] = dx
        lp[1:50001, 2] = dy
        lp[1:50001, 4] = (1/beta) * bucket_length/3 # (cT>0 means trailing)
        lp[50001:, 0] = dx # possibility of differing offsets leading/trailing
        lp[50001:, 2] = dy
        lp[50001:, 4] = -(1/beta) * bucket_length/3 # (cT<0 means leading)
    ...
    for i in range(num_bunches):
        bunch_train_simulator.add_per_turn(i, synergia.bunch.Diagnostics_bulk_track(f"tracks_{i:02d}.h5",
                                                                           4))
        bunch_train_simulator.add_per_turn(i, synergia.bunch.Diagnostics_full2(f"diag_{i:02d}.h5"))

        bunch_train_simulator.add_per_turn(i, synergia.bunch.Diagnostics_particles(f"particles_{i:02d}.h5"))
