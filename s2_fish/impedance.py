...
def apply_impedance_kick(bunch,impedance,tau):
    ...
    rw_kick(rwsize[2],bin_partition,
                    zdensity,xmom,ymom,tau, 
                    bunch.get_store(),
                    pipe_radius,pipe_conduct,cutoff_small_z,wake_coeff, quad_wake_sum, quad_wake)
    
    

class Impedance:
    '''Defines the parameters and methods necessary to  impedance calculation'''
    def __init__(self, pipe_radius, pipe_conduct, wall_thickness, orbit_length, z_grid, pipe_symmetry="circular"):
...
# pipe symmetry keywords so far, see below  "circular", "x parallel plates", "y parallel plates", "elliptical"	
	...
	print "long distance cutoff=", self.cutoff_quad ," x orbit length"
	 # cutoff should be calculated as a function of  wall_thickness
...
# quad_wake_exp is the exponentially decaying term at  distance > cutoff*orbit_length
	r=self.r
	b=self.pipe_radius 
	aa=(synergia.physics_constants.PH_MKS_c*synergia.physics_constants.PH_MKS_eps0)/(sigma*t*r*b) # W(z) is prop to exp(-aa*z) at large z
	...
	print "final quad_wake_sum =", quad_wake_sum
