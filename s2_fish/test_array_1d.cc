...
main()
{
    ...
    Array_1d<double> x2 = x1;
    x2.print("x2");

    std::cout << "making x3 point to data from x1\n";
    Array_1d<double> x3(3,x1.get_data_ptr());
    x3.print("x3");
    ...
    Array_1d<double> x5;
    double *mydata;
    std::vector<int> shape2(vector1(2));
    mydata = (double*)malloc(shape2[0]*sizeof(double));
    x5.reshape(shape2[0],mydata);
    std::vector<int> index(2);
    for (index[0]=0; index[0]<shape2[0]; ++index[0]) {
            x5.at(index[0]) = 2.0*index[0];
    }
    x5.print("x5");
    ...
    std::cout << "attempting to set x5.at(-1)... ";
    try {
        x5.at(-1) = -999;
        std::cout << "no error!\n";
    } catch(std::out_of_range) {
    ...
    std::cout << "x5.offset(1) = " << x5.offset(1) << std::endl; 
